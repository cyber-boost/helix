...
fn handle_connection(mut stream: TcpStream, target_dir: PathBuf) {
...
fn handle_request(request: &str, target_dir: &PathBuf) -> String {
...
fn handle_get_request(path: &str, target_dir: &PathBuf) -> String {
    match path {
...
fn list_binaries(target_dir: &PathBuf) -> String {
...
fn serve_binary_file(filename: &str, target_dir: &PathBuf) -> String {
...
fn create_html_response(html: String) -> String {
    let mut response = String::new();
    response.push_str("HTTP/1.1 200 OK\r\n");
...
fn create_error_response(status: &str, message: &str) -> String {
    let html = format!(
