...
fn lint_project(verbose: bool) -> Result<()> {
...
fn lint_specific_files(files: Vec<PathBuf>, verbose: bool) -> Result<()> {
    if verbose {
...
fn lint_single_file(file: &PathBuf, verbose: bool) -> Result<Vec<LintIssue>> {
...
fn check_line_issues(line: &str, line_num: usize) -> Vec<LintIssue> {
    let mut issues = Vec::new();
...
fn check_file_issues(content: &str, _file: &PathBuf) -> Vec<LintIssue> {
    let mut issues = Vec::new();
