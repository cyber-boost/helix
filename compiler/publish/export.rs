...
fn export_to_json(
    ...
    let export_data = ExportData {
        ...
        dependencies: if include_deps {
            collect_dependencies(project_dir)?
        } else {
            None
        },
...
fn export_to_toml(
    ...
    if include_deps {
        enhanced_manifest.dependencies = Some(collect_dependencies(project_dir)?);
    }
    let toml_content = toml::to_string_pretty(&enhanced_manifest)
...
spec:
  ...
  type: LoadBalancer
"#,
        manifest.package.name, manifest.package.name, manifest.package.name, manifest
        .package.name, manifest.package.name, manifest.package.name, manifest.package
...
fn import_from_toml(input: &PathBuf, force: bool, verbose: bool) -> Result<()> {
    let content = std::fs::read_to_string(input).context("Failed to read input file")?;
    let manifest: super::super::project::init::ProjectManifest = toml::from_str(&content)
...
fn import_export_data(
    ...
    if verbose {
        println!("  ✅ Imported {} source files", export_data.source_files.len());
    }
    Ok(())
}
