...
pub fn clean_project(all: bool, cache: bool, verbose: bool) -> Result<()> {
    ...
    if target_dir.exists() {
        if all {
            ...
            clean_directory(&target_dir, "*.hlxb", &mut cleaned_items)?;
        }
    }
    if cache {
    ...
    if all {
        clean_temp_files(&project_dir, &mut cleaned_items)?;
    }
...
pub fn reset_project(force: bool, verbose: bool) -> Result<()> {
    ...
    let target_dir = project_dir.join("target");
    if target_dir.exists() {
        fs::remove_dir_all(&target_dir).context("Failed to remove target directory")?;
        removed_items.push("target/");
    }
    let lib_dir = project_dir.join("lib");
    if lib_dir.exists() {
        fs::remove_dir_all(&lib_dir).context("Failed to remove lib directory")?;
        removed_items.push("lib/");
    }
    let cache_dir = get_cache_directory()?;
    if cache_dir.exists() {
        fs::remove_dir_all(&cache_dir).context("Failed to remove cache directory")?;
        removed_items.push("cache/");
    }
    clean_temp_files(&project_dir, &mut removed_items)?;
