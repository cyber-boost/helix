...
impl GenericJSONDataset {
    ...
    fn convert_sample_to_training(
        ...
        for (key, value) in obj {
            if !matches!(
                key.as_str(), "prompt" | "chosen" | "rejected" | "completion" | "label" |
        ...
        for sample in &self.samples {
            match &self.format {
                TrainingFormat::Preference { .. } => {
                    if let (Some(chosen), Some(rejected)) = (
                        &sample.chosen,
                        &sample.rejected,
                    ) {
    ...
    fn to_dpo_format(&self) -> Result<DPODataset> {
        ...
        for sample in &self.samples {
            if let TrainingFormat::Preference { .. } = &self.format {
                ...
                bail!(
                    "DPO format requires preference-style data (chosen/rejected fields)"
                );
            }
        }
            ...
            TrainingFormat::Preference { .. } => {
                if *chosen_coverage < 0.9 {
                    report
                        ...
                        .push(
                            format!(
                                "Low chosen field coverage: {:.1}%", chosen_coverage * 100.0
                            ),
                        );
                }
                if *rejected_coverage < 0.9 {
                    report
                        ...
                        .push(
                            format!(
                                "Low rejected field coverage: {:.1}%", rejected_coverage *
                                100.0
                            ),
                        );
                }
                report.overall_score = (chosen_coverage + rejected_coverage
                    + prompt_coverage) / 3.0;
            }
            TrainingFormat::Completion { .. } => {
                if *completion_coverage < 0.9 {
                    report
                        ...
                        .push(
                            format!(
                                "Low completion field coverage: {:.1}%", completion_coverage
                                * 100.0
                            ),
                        );
                }
