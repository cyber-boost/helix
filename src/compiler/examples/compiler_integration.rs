...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let binary = compiler.compile_ast(&ast)?;
    println!("✅ Compiled to binary ({} bytes)", binary.len());
    ...
    println!("✅ Binary loaded successfully ({} bytes)", loaded_binary.len());
    println!("\n6. Decompiling binary:");
    let decompiled_ast = compiler.decompile_binary(&loaded_binary)?;
    println!(
        "✅ Binary decompiled to AST with {} declarations", decompiled_ast.declarations
        .len()
    );
