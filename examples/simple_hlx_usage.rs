...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // 2. Bracket notation access: hlx[section][key]
    println!("2. Using bracket notation...");
    hlx["database"]["host"] = Value::String("localhost".to_string());
    hlx["database"]["port"] = Value::Number(5432.0);
    hlx["database"]["name"] = Value::String("mydb".to_string());

    hlx["server"]["host"] = Value::String("0.0.0.0".to_string());
    hlx["server"]["port"] = Value::Number(8080.0);
